...
HARDHAT_PID=$(lsof -i :${HARDHAT_NODE_PORT} -t 2>/dev/null || echo "")
...
# --- Deploy AnonExam.sol on localhost ---
echo "--- Deploying AnonExam.sol on Hardhat Node ---"
npx hardhat deploy --network localhost || true

# Capture the deployment exit code 
DEPLOY_EXIT_CODE=$?
